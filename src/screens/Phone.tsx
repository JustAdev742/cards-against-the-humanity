...
function PhoneSeat({
  ...
  const [entry, setEntry] = useState<{ code: string; name: string } | null>(
    () => {
      ...
      return initialCode.length === 4 && name
        ? { code: initialCode, name }
        : null;
    },
  );
  const { snapshot, client } = useClient(
    entry?.code ?? null,
    entry?.name ?? "",
  );
...
function JoinForm({
  ...
  return (
    <main className="flex min-h-dvh flex-col px-5 pb-[max(1.5rem,var(--inset-bottom))] pt-[max(1.5rem,var(--inset-top))]">
      ...
      >
        <div>
          <h1 className="m-0 text-3xl font-extrabold tracking-[-0.03em]">
            Join the table
          </h1>
        ...
        <div>
          ...
          <p id="code-help" className="mt-3 text-center text-sm text-ash">
            {touched && !codeReady
              ? "Four letters, exactly as shown on the TV."
              : "Four letters."}
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <input
            ...
            className={
              ...
              (touched && !nameReady
                ? "border-danger"
                : "border-line focus-visible:border-paper")
            }
            placeholder="Sam"
            ref={nameRef}
          />
          <p id="name-help" className="m-0 text-sm text-ash">
            {touched && !nameReady
              ? "Everyone needs a name on the TV."
              : "Up to 14 characters."}
          </p>
        </div>
...
export function Seat({
  ...
  useEffect(() => {
    ...
    if (table.phase === "writing" && !self.isCzar && !self.submitted)
      feedback.nudge();
    if (table.phase === "writing" && self.isCzar) feedback.nudge();
    if (table.phase === "judging" && self.isCzar) feedback.nudge();
    if (table.phase === "roundEnd" && table.winnerId === self.playerId)
      feedback.win();
  ...
  return (
    <>
      ...
      {table.shortHanded && snapshot.status === "connected" && (
        <p
          role="status"
          className="m-0 bg-paper px-4 py-2 text-center text-sm font-bold text-ink"
        >
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        {table.phase === "judging" && (
          <PhoneJudging table={table} self={self} client={client} />
        )}
        {table.phase === "roundEnd" && (
          <PhoneRoundEnd table={table} self={self} client={client} />
        )}
        {table.phase === "gameOver" && (
          <PhoneGameOver table={table} self={self} client={client} />
        )}
      </div>
    </>
  );
...
function headingFor(table: TableView, self: SelfView): string {
  switch (table.phase) {
    ...
    case "writing":
      if (self.isCzar)
        return `Round ${table.round}. You are the Card Czar. Read the card out.`;
        ...
        {table.players.map((player) => (
          ...
          >
            <span className="min-w-0">
              ...
              {player.botBlurb && (
                <span className="mt-0.5 block pl-[2.3rem] text-xs text-ash">
                  {player.botBlurb}
                </span>
              )}
            </span>
            {self.isHost && player.id !== self.playerId && (
              <ConfirmButton
                ...
                onConfirm={() =>
                  client?.send({ type: "kick", playerId: player.id })
                }
      ...
      <div className="mt-auto flex flex-col gap-4">
        {self.isHost ? (
          <>
            ...
            <TargetScore
              ...
              onChange={(targetScore) =>
                client?.send({ type: "setOptions", targetScore })
              }
            />
            <label className="flex cursor-pointer items-start gap-3 text-sm text-ash-bright">
              ...
              />
              Deal in Rando Cardrissian: a random card plays every round. If it
              wins, everyone should feel bad.
            </label>
            <label className="flex cursor-pointer items-start gap-3 text-sm text-ash-bright">
              <input
                type="checkbox"
                ...
                className="mt-0.5 h-5 w-5 accent-white"
              />
              Meritocracy: whoever wins a round judges the next one, instead of
              the job going round the table.
            </label>
            {music?.available && (
          ...
          <p className="m-0 text-sm text-ash">
            {table.deck === "family" ? "Family Edition" : "Full deck"}:{" "}
            {table.deckCounts.white} white cards, {table.deckCounts.black}{" "}
            black. Playing to {table.targetScore}.
          </p>
        )}
      </div>
    </div>
    ...
    return (
      <div className="flex min-h-0 overflow-y-auto overscroll-contain flex-1 flex-col gap-5 px-5 py-6">
          ...
          {self.submitted.map((card, index) => (
            <li key={index}>
              <WhiteCard
                ...
                footer={
                  need > 1 ? (
                    <span className="label text-xs! text-ash-ink!">
                      {index + 1}
                    </span>
                  ) : null
                }
        ...
        {table.waitingOn.length > 0 && (
          <p className="m-0 text-sm text-ash">
            Still to play: {table.waitingOn.join(", ")}.
          </p>
        )}
      </div>
    );
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <p className="label shrink-0 px-5 pt-4">
        {need > 1
          ? `Pick ${need}, in the order they should be read`
          : "Pick your answer"}
      </p>
              ...
              >
                <WhiteCard
                  ...
                  className={
                    "min-h-24 p-4 " +
                    (isPicked
                      ? "outline outline-[3px] outline-offset-[3px] outline-paper"
                      : "")
                  }
                  footer={
                    isPicked ? (
      ...
      <div className="shrink-0 border-t border-line bg-ink px-4 pb-[max(1rem,var(--inset-bottom))] pt-3">
        {snapshot.moveError && (
          <p
            role="alert"
            className="m-0 mb-2 text-center text-sm font-bold text-danger"
          >
            {snapshot.moveError}
          </p>
        )}
                  ...
                  {submission.cards.map((card, index) => (
                    <WhiteCard
                      ...
                      className={
                        "min-h-24 p-4 " +
                        (mine
                          ? "outline outline-2 outline-offset-[5px] outline-ash"
                          : "")
                      }
                      footer={
                  ...
                  >
                    ...
                    <FlipCard
                      ...
                      className={
                        ...
                        (chosen
                          ? "outline outline-[3px] outline-offset-[5px] outline-paper"
                          : "")
                      }
                      front={
                        <div className="flex flex-col gap-1.5">
  ...
  return (
    <div className="flex min-h-0 overflow-y-auto overscroll-contain flex-1 flex-col gap-5 px-5 py-6 pb-[max(1.5rem,var(--inset-bottom))]">
      ...
      <p className="m-0 flex items-center gap-2 text-lg font-extrabold">
        ...
        {isRando
          ? "Rando Cardrissian wins it."
          : `${winner?.name ?? "Nobody"} takes the point.`}
      </p>
      {nextUp && (
        <p className="label m-0">
          {nextUp.id === self.playerId
            ? "You judge the next round"
            : `${nextUp.name} judges next`}
        </p>
      )}
      {(self.isCzar || self.isHost) && (
        <Button
          className="mt-auto w-full"
          onClick={() => client?.send({ type: "nextRound" })}
        >
          Next round
        </Button>
      )}
  ...
  return (
    <div className="flex min-h-0 overflow-y-auto overscroll-contain flex-1 flex-col gap-6 px-5 py-6 pb-[max(1.5rem,var(--inset-bottom))]">
      ...
      {self.isHost && (
        <Button
          className="mt-auto w-full"
          onClick={() => client?.send({ type: "playAgain" })}
        >
          Play again
        </Button>
      )}
